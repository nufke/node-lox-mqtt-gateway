...
Adaptor.prototype.publish_mqtt_structure = function() {
    ...
    categories.forEach((category, index) => {
        let id = device_info + '-' + category.uuid;
        this.mqtt_structure.categories[id] =
        {
            hwid: device_info,
            uuid: category.uuid,
            name: category.name,
            type: category.type,
            icon: { href: category.image },
    ...
    controls.forEach((control, index) => {
        let id = device_info + '-' + control.uuidAction;

        if (control.defaultIcon != null) {
            icon = control.defaultIcon;
            if (icon.search(".svg") == -1) // ext not found
              icon = icon + ".svg";
        }
        else // take icon from category
          icon = this.mqtt_structure.categories[device_info + '-' + control.category].icon.href;
        ...
        this.mqtt_structure.controls[id] =
        {
            uuid: device_info,
