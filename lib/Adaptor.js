...
Adaptor.prototype.get_command_from_topic = function(topic, data) {
    ...
    /* virtual controls use web services over http, not websocket */
    var protocol = (control.type === 'UpDownDigital') ? 'http' : 'ws';
...
Adaptor.prototype.get_topics_for_subscription = function() {
    // topics: mqtt_prefix_lox/serialnr/uuid/cmd, mqtt_prefix_lox/serialnr/uuid/<subcontrol>/cmd
    return [this.mqtt_prefix_lox + '/+/+/cmd', this.mqtt_prefix_lox + '/+/+/+/cmd'];
};
...
Adaptor.prototype.publish_mqtt_structure = function() {
    ...
    Object.keys(categories).forEach( key => {
        ...
        this.mqtt_structure.categories[mqtt_uuid] =
        {
            ...
            uuid: category.uuid,
            mqtt_cmd: this.mqtt_prefix_lox + '/' + mqtt_uuid + '/cmd',
            name: category.name,
            type: category.type,
    ...
    Object.keys(controls).forEach( key => {
        ...
        // default: take icon from category
        let icon = Object.values(this.mqtt_structure.categories).find(element => element.uuid === control.category).icon.href;
        ...
        if (control.defaultIcon && (control.defaultIcon.length>0)) {
          ...
          if (control.type === 'Webpage') {
              icon = this.icon_path + '/00000000-0000-0019-2200000000000000.svg'; // Note, logo from old series
          }
        }
                ...
                subcontrols[sub_uuid] =
                    {
                        ...
                        mqtt_cmd: this.mqtt_prefix_lox + '/' + sub_uuid + '/cmd',
        ...
        Object.keys(obj.items).forEach(function (key) {
            if (Array.isArray(obj.items[key].uuid)) { // handle array,
                ...
                obj.items[key].uuid.forEach(uuid => {
                    list.push({ mqtt: that.mqtt_prefix_lox + '/' + that.device_info + '/' + uuid })});
                states[camelToSnake(key)] = list;
            }
            else
                states[camelToSnake(key)] = { mqtt: that.mqtt_prefix_lox + '/' + that.device_info + '/' + obj.items[key].uuid };
        });
    }
    return states;
}
...
Adaptor.prototype._add_control = function(control) {
    ...
    var path = this.mqtt_prefix_lox + '/' + serialnr + '/' + control.uuidAction;
