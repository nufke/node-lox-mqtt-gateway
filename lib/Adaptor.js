...
Adaptor.prototype.publish_mqtt_structure = function() {
    ...
    categories.forEach((category) => {
        this.mqtt_structure.categories.push(
        {
            ...
            uuid: category.uuid,
            mqtt: {
                subscribe_topic: this.mqtt_prefix_lox + '/' + device_info + '/' + category.uuid,
                command_topic: this.mqtt_prefix_lox + '/' + device_info + '/' + category.uuid + '/cmd',
                qos: 1,
                retain: true
            },
            name: category.name,
    ...
    rooms.forEach((room) => {
        this.mqtt_structure.rooms.push(
        {
            ...
            uuid: room.uuid,
            mqtt: {
                subscribe_topic: this.mqtt_prefix_lox + '/' + device_info + '/' + room.uuid,
                command_topic: this.mqtt_prefix_lox + '/' + device_info + '/' + room.uuid + '/cmd',
                qos: 1,
                retain: true
            },
            name: room.name,
    ...
    controls.forEach((control, index) => {
        ...
        if (control.states !== undefined)
          Object.keys(control.states.items).forEach( function(key) {
            states[camelToSnake(key)] = that.mqtt_prefix_lox + '/' + device_info + '/' + control.states.items[key].uuid } );
        ...
        this.mqtt_structure.controls.push(
        {
            ...
            mqtt: {
                subscribe_topic: this.mqtt_prefix_lox + '/' + device_info + '/' + control.uuidAction,
                command_topic: this.mqtt_prefix_lox + '/' + device_info + '/' + control.uuidAction + '/cmd',
                qos: 1,
