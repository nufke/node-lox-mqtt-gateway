...
Adaptor.prototype.publish_mqtt_structure = function() {
    ...
    Object.keys(categories).forEach( key => {
        ...
        this.mqtt_structure.categories[mqtt_uuid] =
        {
            ...
            order: 11 - category.defaultRating
        };
    });
    ...
    Object.keys(controls).forEach( key => {
        ...
        this.mqtt_structure.controls[mqtt_uuid] =
        {
            ...
            order: 11-control.defaultRating,
            homepage_order: control.isFavorite ? (11 - control.defaultRating) : null /* rating 0 should be lowest */
        };
    });
        ...
        Object.keys(obj.items).forEach(function (key) {
            if (Array.isArray(obj.items[key].uuid)) { // handle array,
                ...
                obj.items[key].uuid.forEach(uuid => {
                    list.push({ value: "", mqtt: that.mqtt_prefix_lox + '/' + that.device_info + '/' + uuid })});
                states[camelToSnake(key)] = list;
            }
            else
                states[camelToSnake(key)] = { value: "", mqtt: that.mqtt_prefix_lox + '/' + that.device_info + '/' + obj.items[key].uuid };
        });
    }
    return states;
}
